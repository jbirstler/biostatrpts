  ...
  for (i in 1:length(varvec)) {
    ...
    for (j in 1:length(VisitNameLevels)) {
      ...
      if (length(vispts) != length(unique(vispts))) {
        ...
        warning(paste(
          "These patients", paste(multi, collapse = ", "), "had more than 1 ", VisitNameLevels[j],
          " measure"
        ))
      }
    }
    ...
    fullData <- data.frame(visit.id = levels(as.factor(thisData$visit):as.factor(thisData$id)), var = NA)
    fullData$visit <- as.factor(unlist(lapply(
      strsplit(as.character(fullData$visit.id), ":", fixed = TRUE),
      "[[", 1
    )))
    fullData$id <- as.factor(unlist(lapply(
      strsplit(as.character(fullData$visit.id), ":", fixed = TRUE),
      "[[", 2
    )))
