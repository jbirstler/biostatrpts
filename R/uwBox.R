...
uwBox <- function(data, trxName = NULL, metricName, yLab = NULL, xLab = NULL, yLim = NULL, boxWex = 0.75,
  ...
  # Calculate values for box plot whiskers and box values Whisker calculated as largest observation <=
  # 75th + 1.5xIQR or smallest observation >= 25th + 1.5xIQR Box calculated as 25th and 75th percentile
  if (is.null(trxName)) {
    ...
    lWhisker <- ifelse(sum(data[[metricName]] < (q25 - 1.5 * iqr)) == 0, mins, ifelse(sort(data[[metricName]])[(sum(data[[metricName]] <
      (q25 - 1.5 * iqr)) + 1)] > q25, q25, sort(data[[metricName]])[(sum(data[[metricName]] < (q25 -
      1.5 * iqr)) + 1)]))
    uWhisker <- ifelse(sum(data[[metricName]] > (q75 + 1.5 * iqr)) == 0, maxs, ifelse(sort(
      data[[metricName]],
      TRUE
    )[(sum(data[[metricName]] > (q75 + 1.5 * iqr)) + 1)] < q75, q75, sort(
      data[[metricName]],
      TRUE
    )[(sum(data[[metricName]] > (q75 + 1.5 * iqr)) + 1)]))
    ...
    for (i in 1:length(q25)) {
      ...
      lWhisker[i] <- ifelse(sum(thisTrx < (q25[i] - 1.5 * iqr[i])) == 0, mins[i], ifelse(sort(thisTrx)[(sum(thisTrx <
        (q25[i] - 1.5 * iqr[i])) + 1)] > q25[i], q25[i], sort(thisTrx)[(sum(thisTrx < (q25[i] -
        1.5 * iqr[i])) + 1)]))
      uWhisker[i] <- ifelse(sum(thisTrx > (q75[i] + 1.5 * iqr[i])) == 0, maxs[i], ifelse(sort(
        thisTrx,
        TRUE
      )[(sum(thisTrx > (q75[i] + 1.5 * iqr[i])) + 1)] < q75[i], q75[i], sort(thisTrx, TRUE)[(sum(thisTrx >
        (q75[i] + 1.5 * iqr[i])) + 1)]))
    }
  ...
  if (pOutliers) {
    if (is.null(trxName)) {
      if (length(data[[metricName]][data[[metricName]] < lWhisker | data[[metricName]] > uWhisker]) >
        0) {
        points(x = rep(1, length(data[[metricName]][data[[metricName]] < lWhisker | data[[metricName]] >
          uWhisker])), y = data[[metricName]][data[[metricName]] < lWhisker | data[[metricName]] >
          uWhisker], pch = 1)
      }
    } else {
      for (i in 1:length(lWhisker)) {
        ...
        if (length(thisTrx[thisTrx < lWhisker[i] | thisTrx > uWhisker[i]]) > 0) {
          points(x = rep(i, length(thisTrx[thisTrx < lWhisker[i] | thisTrx > uWhisker[i]])), y = thisTrx[thisTrx <
            lWhisker[i] | thisTrx > uWhisker[i]], pch = 1, cex = 0.6)
        }
      }
    }
  }
