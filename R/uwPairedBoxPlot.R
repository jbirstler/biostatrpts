...
#'   label = NULL
#' )
uwPairedBoxPlot <- function(allData, pairName, metricName, ptID, onlyPairs = TRUE, plotDiff = TRUE, plotDiffOnly = FALSE,
                            pctChange = FALSE, chgLayout = TRUE, pWilcox = TRUE, LatexFileName = NULL, pTitle = NULL, yLab = NULL,
                            plotMean = FALSE, pOutliers = FALSE, lWhisker = 0.05, hWhisker = 0.95, lHinge = 0.25, hHinge = 0.75,
                            boxWex = 0.75, printPVals = TRUE, yLim = NULL, footNote = NULL, firstCol.name = NULL, LatexCaption = NULL,
                            showTab = FALSE, ...) {
  ...
  allData <- subset(allData, select = c(pairName, ptID, metricName))
  ...
  pairedData <- subset(allData, subset = (allData[[ptID]] %in% eligibleIDs))
  ...
  diffVec <- pairedData[[metricName]][((totN / 2) + 1):totN] - pairedData[[metricName]][1:(totN / 2)]
  ...
  if (onlyPairs) {
    bp1 <- boxplot(pairedData[[metricName]] ~ pairedData[[pairName]], axes = FALSE, plot = FALSE)
    Nstr <- rep(paste("n = ", totN / 2, sep = ""), 2)
    tableN <- rep(totN / 2, 2)
    allData <- pairedData # so only one set of code the rest of the way
  } else {
    bp1 <- boxplot(allData[[metricName]] ~ allData[[pairName]], axes = FALSE, plot = FALSE)
  ...
  quantile05.values <- tapply(allData[[metricName]], list(allData[[pairName]]), quantile,
    probs = c(lWhisker),
    na.rm = TRUE
  )
  ...
  quantile95.values <- tapply(allData[[metricName]], list(allData[[pairName]]), quantile,
    probs = c(hWhisker),
    na.rm = TRUE
  )
  ...
  quantile25.values <- tapply(allData[[metricName]], list(allData[[pairName]]), quantile,
    probs = c(lHinge),
    na.rm = TRUE
  )
  ...
  quantile75.values <- tapply(allData[[metricName]], list(allData[[pairName]]), quantile,
    probs = c(hHinge),
    na.rm = TRUE
  )
  ...
  std.values1 <- round(tapply(allData[[metricName]], list(allData[[pairName]]), sd, na.rm = TRUE), 2)

  min.values1 <- round(tapply(allData[[metricName]], list(allData[[pairName]]), min, na.rm = TRUE), 2)

  max.values1 <- round(tapply(allData[[metricName]], list(allData[[pairName]]), max, na.rm = TRUE), 2)
  ...
  if (printPVals) {
    if (pWilcox) {
      ...
      pVal <- round(
        t.test(pairedData[[metricName]] ~ pairedData[[pairName]], paired = TRUE, exact = FALSE)$p.value,
        3
      )
    }
    if (pVal == 0) {
  ...
  mean.values1 <- round(tapply(allData[[metricName]], allData[[pairName]], mean, na.rm = TRUE), 2)
  ...
  if (!plotDiffOnly) {
    if (plotMean) {
      ...
      bxp(bp1, ylab = yLab, xaxt = "n", boxwex = boxWex, outline = pOutliers, axes = FALSE, ylim = yLim)

      # Now restore the proper median values
      bp1$stats[3, ] <- median.values1
    } else {
      bxp(bp1,
        ylab = yLab, xaxt = "n", boxwex = boxWex, outline = pOutliers, axes = FALSE, ylim = yLim,
        medlty = "blank"
      )
    }
  ...
  LatexTab <- cbind(
    Pair = pairVector, N = tableN, Mean = mean.values1, SD = std.values1, Min = min.values1,
    P5 = bp1$stats[1, ], Q1 = bp1$stats[2, ], Median = median.values1, Q3 = bp1$stats[4, ], P95 = bp1$stats[5, ], Max = max.values1
  )
  ...
  if (plotDiff) {
    ...
    diffSummary <- c(diffName, totN / 2, mean.values2, std.values2, min.values2, bp2$stats[1, ], bp2$stats[2, ], median.values2, bp2$stats[4, ], bp2$stats[5, ], max.values2)
