...
uwLeveledBoxPlot <- function(allData,
  ...
  if (delta) {
    if (!is.null(hifactName)) {
      ...
      for (i in 1:length(lowfactVec))
      {
        ...
        diffDat <- diffFunction(
          Diffdata = thisLowFact, variable = metricName,
      ...
      allData <- diffFunction(
        Diffdata = allData, variable = metricName,
        TrxName = trxName, Baseline = baseLevel,
  ...
  bpFunc <- function(var) {
    ...
    return(list(
      bp = bp, quantile.05.values = quantile05.values,
  ...
  if (delta) {
    if (length(median.values) <= 12) {
      points(
        x = boxPlace[-c(1:baseLevelNboxes)],
  ...
  if (printPVals) {
    if (is.null(hifactName)) {
      pvals <- uwPVals(
        data = allData, factNames = lowfactName, trxName = trxName, pAdjust = pAdjust,
      ...
      pvals <- uwPVals(
        data = allData, factNames = c(hifactName, lowfactName), trxName = trxName,
    ...
    if (pairwise == FALSE & PVinTableOnly == FALSE) {
      ...
      if (!PVinTableOnly) {
        mtext("p", side = 3, at = nPlace, line = 0.1, cex = PValCex)
      }
      if (!is.null(trxControl) & is.null(pInclude)) {
        if (!PVinTableOnly) {
          mtext(pvals$pv, side = 3, at = boxPlace, line = 0, cex = PValCex)
        }
      }
      else # This encompasses all pairwise or when using pInclude
      {
        if (!PVinTableOnly) {
          mtext("contrast", side = 3, at = nPlace, line = 0.75, cex = PValCex)
        }
  ...
  if (printPVals) {
    if (!pvtable) {
      ...
      if (is.null(LatexFileName)) {
        ...
        ltxPVTbl <- uwLatex(
          mat = pval.table, file = pvtabFileName,
  ...
  if (!is.null(hifactName)) {
    ...
    if (is.vector(table.values)) {
      table.values <- matrix(table.values,
        nrow = 1,
  ...
  if (is.null(LatexFileName)) {
    ...
    if (is.null(LatexCaption)) {
      ltxTbl <- uwLatex(
        mat = table.values, file = LatexFileName, caption = pTitle,
        col.just = col.just, ...
      )
    }
    else {
      ltxTbl <- uwLatex(
        mat = table.values, file = LatexFileName, caption = LatexCaption,
        col.just = col.just, ...
      )
    }
  }
