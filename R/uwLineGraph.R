...
uwLineGraph <- function(data,
                        ...
                        LegendCex=0.7,
                        printPVals=TRUE,
                        pTest=c("wilcox","t", "kruskal", "anova"),
                        pExclude=NULL,
                        trxControl=NULL,
                        pairDiffTab=FALSE,
                        ptIDname=NULL,
                        ...
                        ...)
  {
    ...
    #load library to get drop.levels function
    library(gdata) 
    ...
    if(is.logical(colScale))
      {
        if(!colScale)
          ...
          {
            color <- c("black","red","blue","forestgreen","gold2","violet","brown",
                       "chartreuse","lightgreen","magenta")[1:NtrxLevs]
          }
      }
    ...
    if(is.null(yLim))
      ...
      {
        if(yLim[1] > minY | yLim[2] < maxY){warning("yLim doesn't cover all points and has been changed")}
        yLim[1] <- ifelse(yLim[1] > minY,minY,yLim[1])
        yLim[2] <- ifelse(yLim[2] < maxY,maxY,yLim[2])
      }   
    plot(x=1:NfactLevs,
         y=if(Ropen){plotPts}else{plotPts[grep(paste("^",trxLevs[1],sep=""),names(plotPts))]},
         lty=ltys[1],ylim=yLim,main=pTitle, type="b",xlab=xLab, ylab=yLab,las=1,
         pch=rep(pchPts[1],NtrxLevs),col=rep(color[1],NtrxLevs), xaxt="n")
    ...
    if(!Ropen)
      {
        for(i in 2:NtrxLevs)
          {
            points(x=1:NfactLevs, y=plotPts[grep(paste("^",trxLevs[i],sep=""),names(plotPts))],
                   lty=ltys[i],pch=rep(pchPts[i],NtrxLevs),col=rep(color[i],NtrxLevs),type="b")
          }
        #add legend
        if(Legend)
          {
            legend(LegendLoc, col=color, pch=pchPts, lty=ltys,
                   legend=trxLevs, bg="white", cex=LegendCex)
          }
      }
    ...
    Vals25th <- tapply(dat[[metricName]],dat[[factorName]]:dat[[trxName]],quantile,prob=.25)
    Vals75th <- tapply(dat[[metricName]],dat[[factorName]]:dat[[trxName]],quantile,prob=.75)
    
    mat1 <- round(cbind("N"=N,"Mean"=meanVals,"SD"=sdVals,"Min."=minVals,"Q1"=Vals25th,
                        "Median"=medVals,"Q3"=Vals75th,"Max."=maxVals),numDec)
    rownames(mat1) <- NULL
    if(Ropen)
      {
    ...
    if(pairDiffTab)
      {
        ...
        diff <- diffFunction(dat, variable=metricName, TrxName=trxName, VisitName=factorName,
                             ptID=ptIDname, Baseline=BaseLevel)
        ...
        diff25th <- tapply(diff$diffCol, diff$timeCol:diff$diffTRT,quantile,prob=.25)
        diff75th <- tapply(diff$diffCol, diff$timeCol:diff$diffTRT,quantile,prob=.75)
        
        matDiff <- round(cbind("N"=Ndiff,"Mean"=meandiff,"SD"=sddiff,"Min."=mindiff,"Q1"=diff25th,
                               "Median"=meddiff,"Q3"=diff75th,"Max."=maxdiff),numDec)
    ...
    if(printPVals)
      {
        ...
        if(pairDiffTab)
          {
            pvals <- uwPVals(data=diff, trxName="diffTRT",trxControl=trxControl,
                             metricName="diffCol",factNames="timeCol", pTest=pTest,
                             pExclude=pExclude, pairwise=pairwise)
            if(!pairwise)
              {
          ...
          {
            pvals <- uwPVals(data=dat,trxName=trxName,trxControl=trxControl,pTest=pTest,
                             metricName=metricName,factNames=factorName,pExclude=pExclude,
                             pairwise=pairwise)
    ...
    if(is.null(LatexFileName))
      ...
      {
        tab1 <- uwLatex(mat=mat1, file=LatexFileName[1], cgroup=NULL, n.cgroup=NULL,
                        rowLines=unlist(rowLines[1]), caption=unlist(caption[1]), ...)
      }
    if(pairDiffTab)
      {
        if(is.null(LatexFileName))
        ...
        else
          {
            ...
            tab2 <- uwLatex(mat=matDiff, file=LatexFileName[2], cgroup=NULL, n.cgroup=NULL,
                            caption=ifelse(is.null(unlist(caption[2])),
                              paste("Paired Difference Table for",trxName,"by",factorName),
                              unlist(caption[2])),
